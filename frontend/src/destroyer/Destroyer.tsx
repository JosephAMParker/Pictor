...
import { Button, CircularProgress, SvgIconTypeMap, styled } from '@mui/material';  
...
const createStyledIcon = (IconComponent: OverridableComponent<SvgIconTypeMap<{}, "svg">> & { muiName: string; }, paddingTop: string) => styled(IconComponent)({
...
const Destroyer = () => {    
        ...
        function checkCracks() {
            ...
            if(workerGreenLight){
                if(canvas && ctx){
                    ...
                    worker.postMessage({ data:pixelData, width:canvas.width, height:canvas.height });
      ...
      if (canvasRef.current) {
        ...
        html2canvas(body, {scale:1}).then((htmlCanvas) => {  
            ...
            if(canvas && context){
                ...
                context.drawImage(
                    htmlCanvas,
                    old_scrollX, old_scrollY, innerWidth, innerHeight,  // Source rectangle
                    0, 0, innerWidth, innerHeight                       // Destination rectangle
                );
    ...
    const startDestroyer = () => { 
        ...
        const iframe = document.getElementById('neowise-frame') as HTMLIFrameElement;
    ...
    return (
        ...
        <> 
            ...
            {!modeOn &&  
                <StartButton data-html2canvas-ignore="true" onClick={startDestroyer} color="inherit" sx={{ flexGrow: 2 }}>
                    <StartButtonContent>
            ...
            {modeOn &&  
                <> 
                    <StartButton data-html2canvas-ignore="true" onClick={closeDestroyer} color="inherit" sx={{ flexGrow: 2 }}>
                        <StartButtonContent>
                            <StyledCloseIcon /> 
                        </StartButtonContent>
                    </StartButton>

                    <RefreshButton data-html2canvas-ignore="true" onClick={restartDestroyer} color="inherit" sx={{ flexGrow: 2 }}>
                        <StartButtonContent>
                            <StyledReplayIcon /> 
                        </StartButtonContent>
                    </RefreshButton> 
