...
def main(bhs: BlackHoleSolver, theta_deg):
    ...
    for x in range(w): 
        print(x, '/', w)
        for y in range(h-1, 0, -1): # iterate in reverse to allow checking primary value when getting secondary 
            
            scl = 1
            _x = x - w//2
            _y = y - h//2
            a,b = bhs.cartesian_to_polar(_x*scl, _y*scl)  
            yPos = y
            xPos = x
...
# scaled_primary
    ...
    cv2.imwrite("bh-" + str(theta_deg) + ".png", (scaled_final * 255).astype("uint8"))
    # fin_newt = cv2.resize(scaled_newt, (fin_w, fin_h), interpolation=cv2.INTER_LANCZOS4)
    cv2.imwrite("newtonian-" + str(theta_deg) + ".png", (scaled_newt * 255).astype("uint8"))  
    ...
    cv2.imwrite("primary-" + str(theta_deg) + ".png", scaled_primary_with_alpha) 
    ...
    cv2.imwrite("ghost-" + str(theta_deg) + ".png", scaled_ghost_with_alpha) 
    ...
    cv2.imwrite("ring-" + str(theta_deg) + ".png", scaled_ring_with_alpha)
